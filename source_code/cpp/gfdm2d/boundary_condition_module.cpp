...
void allocateBoundaryConditions(Mesh &mesh,
                                structModelInput &modelInput,
                                Eigen::Array<unsigned int, Eigen::Dynamic, 1> &nodeTypes,
                                Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &boundAllocMat)
{
    nodeTypes.resize(numBoundNodes);
    ...
    for (int i = 0; i < numBoundNodes; i++)
    {
        ...
        else if (indices.size() == 2)
        {
            ...
            else if (boundType1 == 1 && boundType2 != 1)
            {
            ...
            else if (boundType1 == 2 && boundType2 == 3)
            {
                boundAllocMat(i,indices[1]) = 0;
                nodeTypes(i) = 2;
            }
            else if (boundType1 == 3 && boundType2 == 2)
            {
                boundAllocMat(i,indices[0]) = 0;
                nodeTypes(i) = 2;
            }
